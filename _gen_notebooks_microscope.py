#!/usr/bin/env python
"""Generate microscope-image CNN notebook (BloodMNIST / MedMNIST)."""
import json
import os
...
C1 = '''# ============================================================
# Setup & download of public microscope dataset (MedMNIST)
# ============================================================
import warnings
...
# ---- BloodMNIST: real microscope blood-cell images (open-access dataset) ----
...
print(f"Image shape: {np.array(train_ds[0][0]).shape} (RGB) | Classes: {len(class_names)}")
for i, n in enumerate(class_names):
    print(f"  class {i}: {n}")'''

C2 = '''# ============================================================
# Visualize one example per cell type
# ============================================================
...
for ax in axes.flat:
    for img, label in train_ds:
        ...
        shown.add(c)
        ax.imshow(img)
        ax.set_title(f"{c}: {class_names[c]}", fontsize=8)
        ax.axis("off")
        break
plt.suptitle("BloodMNIST — microscope blood-cell images (28x28 RGB)")
plt.tight_layout(); plt.show()'''

C3 = '''# ============================================================
# DataLoaders + CNN architecture
# ============================================================
...
class BloodCNN(nn.Module):
    """Conv2D -> ReLU -> MaxPool -> Conv2D -> ReLU -> MaxPool -> FC (3-channel input)."""
...
sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",
            ...
            yticklabels=[f"{i}\\n{n.split()[0]}" for i, n in enumerate(class_names)])
plt.title("BloodMNIST CNN — confusion matrix")
...
notebook = [
    md("# 🧬 CNN on Microscope Images — Blood Cell Classification (PyTorch)\n"
       "**Public dataset:** BloodMNIST (MedMNIST) — real microscope bright-field "
       "images of 8 blood-cell types (basophil, eosinophil, erythroblast, immature "
       "granulocytes, lymphocyte, monocyte, neutrophil, platelet).\n"
       "This is the same workflow used for cell counting / differential diagnosis "
...
path = os.path.join(BASE, "10_CNN_Microscope.ipynb")
with open(path, "w", encoding="utf-8") as f:
    json.dump(make_notebook(notebook), f, indent=1, ensure_ascii=False)
print("Wrote 10_CNN_Microscope.ipynb")
